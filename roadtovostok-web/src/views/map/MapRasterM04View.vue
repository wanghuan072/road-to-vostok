<template>
  <article class="map-page page-article map-raster-page">
        ...
        <div class="map-raster-hero-row">
          <div class="page-hero-content map-raster-hero-row__main">
            <nav class="page-hero-breadcrumb" :aria-label="$t('site.breadcrumbAriaLabel')">
              ...
              <span aria-hidden="true">/</span>
              <span>{{ $t('mapRasterM04.breadcrumbCurrent') }}</span>
            </nav>
            <h1>{{ $t('mapRasterM04.title') }}</h1>
            <p v-html="$t('mapRasterM04.introHtml')"></p>
          </div>
          <MapRasterHeroMosaic />
        </div>
      </div>
      ...
      <div class="container">
        <header class="map-section-head">
          <div class="map-section-head-main">
            <p class="map-section-kicker">{{ $t('mapRasterM04.sectionKicker') }}</p>
            <h2>{{ $t('mapRasterM04.sectionTitle') }}</h2>
            <p class="map-hint" v-html="$t('mapRasterM04.hintHtml')"></p>
          </div>
          <ul class="map-legend map-legend--village" :aria-label="$t('mapRasterM04.legendAria')">
            ...
            <div class="map-map-row">
              <div class="map-stage">
                ...
                <aside
                  ...
                  :aria-label="$t('mapRasterM04.railAria')"
                >
                  <div class="poi-rail-head village-filter-rail__head">
                    <span class="poi-rail-kicker">{{ $t('mapRasterM04.railKicker') }}</span>
                    <h3 class="poi-rail-title">{{ $t('mapRasterM04.railTitle') }}</h3>
                    <p class="poi-rail-lead" v-html="$t('mapRasterM04.railLeadHtml')"></p>
                  </div>
        ...
        <div class="map-village-below" v-html="$t('mapRasterM04.belowMapHtml')"></div>
    ...
    <section class="page-body-section">
      <div class="container">
        <p class="guide-disclaimer" v-html="$t('mapRasterM04.footnoteHtml')"></p>
        <p class="guide-tools map-next-link" v-html="$t('mapRasterM04.crossLinksHtml')"></p>
      </div>
    </section>
  </article>
</template>
...
import map04 from '../../data/map/maps/map04.js'
...
} = useRasterMapPage(map04, { coordHud })
</script>
