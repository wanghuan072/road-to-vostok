<template>
  <article class="map-page page-article map-raster-page">
        ...
        <div class="map-raster-hero-row">
          <div class="page-hero-content map-raster-hero-row__main">
            <nav class="page-hero-breadcrumb" :aria-label="$t('site.breadcrumbAriaLabel')">
              ...
              <span aria-hidden="true">/</span>
              <span>{{ $t('mapRasterM02.breadcrumbCurrent') }}</span>
            </nav>
            <h1>{{ $t('mapRasterM02.title') }}</h1>
            <p v-html="$t('mapRasterM02.introHtml')"></p>
          </div>
          <MapRasterHeroMosaic />
        </div>
      </div>
      ...
      <div class="container">
        <header class="map-section-head">
          <div class="map-section-head-main">
            <p class="map-section-kicker">{{ $t('mapRasterM02.sectionKicker') }}</p>
            <h2>{{ $t('mapRasterM02.sectionTitle') }}</h2>
            <p class="map-hint" v-html="$t('mapRasterM02.hintHtml')"></p>
          </div>
          <ul class="map-legend map-legend--village" :aria-label="$t('mapRasterM02.legendAria')">
            ...
            <div class="map-map-row">
              <div class="map-stage">
                ...
                <div
                  ...
                  class="rtv-map-host"
                  role="application"
                  :aria-label="$t('mapRasterM02.mapAppAria')"
                  tabindex="0"
                />
                <div
                  v-show="mapReady"
                ...
                <aside
                  ...
                  :aria-label="$t('mapRasterM02.railAria')"
                >
                  <div class="poi-rail-head village-filter-rail__head">
                    <span class="poi-rail-kicker">{{ $t('mapRasterM02.railKicker') }}</span>
                    <h3 class="poi-rail-title">{{ $t('mapRasterM02.railTitle') }}</h3>
                    <p class="poi-rail-lead" v-html="$t('mapRasterM02.railLeadHtml')"></p>
                  </div>
        ...
        <div class="map-village-below" v-html="$t('mapRasterM02.belowMapHtml')"></div>
    ...
    <section class="page-body-section">
      <div class="container">
        <p class="guide-disclaimer" v-html="$t('mapRasterM02.footnoteHtml')"></p>
        <p class="guide-tools map-next-link" v-html="$t('mapRasterM02.crossLinksHtml')"></p>
      </div>
    </section>
  </article>
</template>
...
import map02 from '../../data/map/maps/map02.js'
...
} = useRasterMapPage(map02, { coordHud })
</script>
